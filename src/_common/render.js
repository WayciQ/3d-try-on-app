...
function setTheLights() {
  ...
  var lightAmb = new THREE.AmbientLight(0xd1d1e0);
  scene.add(lightAmb);
}
...
export function IntializeThreejs(objName, containerRef, videoRef) {
  ...
  renderer.setSize(
    windowWidth,
    (windowWidth * video.videoHeight) / video.videoWidth
  );
...
async function renderPrediction() {
  ...
  if (predictions.length > 0) {
    ...
    for (let i = 0; i < predictions.length; i++) {
      ...
      const faceBasePos = new THREE.Vector3(
        -points[168][0],
        -points[5][1],
        -points[10][2] - 190
      );
      ...
      const rotFactor = windowWidth * 1.5;
      ...
      const faceScale = scaleFactor * 1.1;
